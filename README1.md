...
- Três campos técnicos de metadados são adicionados:
  ...
  - `source`
  - `page`
- O resultado é gravado como JSON em um caminho versionado usando `run_ts`
...
A DAG passa `{{ ts_nodash }}` para cada script. Esse valor é usado como `run_ts`, permitindo que cada execução do pipeline grave em um diretório único.

Essa estratégia versionada evita conflitos de sobrescrita, preserva o histórico de execuções e melhora a rastreabilidade.
...
O projeto usa PySpark localmente por simplicidade. Leituras/escritas diretas em armazenamento de objetos na nuvem exigiriam configuração adicional de conectores do Spark.
